...
Pushing a [GitHub Release](../../releases) triggers
`.github/workflows/release.yml`, which:
1. Sets `version` in both `edge/manifest.json` and `firefox/manifest.json`
   to match the release tag (`v1.2.0` → `1.2.0`).
2. Zips each folder with `manifest.json` at the zip root.
3. Attaches `meet-roll-attendance-edge-<tag>.zip` and
   `meet-roll-attendance-firefox-<tag>.zip` to the release automatically.

Just tag and publish a release (e.g. `v1.1.0`) and both zips show up as
downloadable assets — no manual zipping needed.
...
AMO's automated linter will still show a handful of **warnings** (not
errors) on `libs/jspdf.umd.min.js` and `libs/jspdf.plugin.autotable.min.js`
— things like "unsafe assignment to innerHTML" and "unsafe call to
document.write". These come from jsPDF's own bundled fallback code paths
(used for opening a PDF in a new browser tab on very old browsers), which
this extension never calls — it only uses `doc.save()` and `doc.autoTable()`.
These warnings are expected for any extension bundling jsPDF and don't block
submission; you can mention this in the reviewer notes if asked.
